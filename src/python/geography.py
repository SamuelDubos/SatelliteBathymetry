...
def clouds_percentage(xml_):
    """Gives the percentage of pixels that contain no data."""
    with open(xml_, 'r') as f:
        doc = BeautifulSoup(f, 'xml')
        medium = doc.MEDIUM_PROBA_CLOUDS_PERCENTAGE.string
        high = doc.HIGH_PROBA_CLOUDS_PERCENTAGE.string
        cirrus = doc.THIN_CIRRUS_PERCENTAGE.string
    return round(float(medium) + float(high) + float(cirrus), 2)
...
def transform_p2c(x_, y_, borders_, size_):
    """Transforms a given location in Mercator-projection to (x, y) coordinates in a plan image."""
    est_, west_, south_, north_ = [borders_[i] for i in range(4)]
    lat_ = x_ * (south_ - north_) / size_[0] + north_
    lng_ = y_ * (est_ - west_) / size_[1] + west_
    return lat_, lng_
